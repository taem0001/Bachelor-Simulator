...
namespace Simulator {
	int _blt_instruction(const Register &rs1, const Register &rs2, const int imm) {
		...
		if (!rs1_unsigned && !rs2_unsigned) {
			...
			if (rs1_signed < rs2_signed) return imm;
			return 0;
		}
		...
		if (rs1_unsigned && !rs2_unsigned) {
			...
			if (rs1.data < static_cast<uint32_t>(rs2_signed)) return imm;
			return 0;
		}

		int32_t rs1_signed = static_cast<int32_t>(rs1.data);
		if (rs1_signed < 0) return imm;
		if (static_cast<uint32_t>(rs1_signed) < rs2.data) return imm;
		return 0;
	}

	int _bge_instruction(const Register &rs1, const Register &rs2, const int imm) {
		const bool rs1_unsigned = is_unsigned(rs1.tag);
	...
	void CPU::b_instruction(const char imm4_1_11, const char func3, const char rs1, const char rs2,
		...
		case 0x1: // bne
			...
			break;
		case 0x4: // blt
			pc += _blt_instruction(registers[rs1], registers[rs2], imm);
			break;
		case 0x5: // bge
            pc += _bge_instruction(registers[rs1], registers[rs2], imm);
			break;
		default:
			break;
		}
