...
namespace Simulator {
	...
	void CPU::s_instruction(const char imm4_0, const char func3, const char rs1, const char rs2, const char imm11_5) {
		...
		int32_t imm = (static_cast<int32_t>(imm11_5 & 0x7F) << 5) | static_cast<int32_t>(imm4_0 & 0x1F);
		...
		if (temp_addr < 0 || temp_addr >= MEMORY_SIZE_BYTES) {
			...
			std::cerr << "  Base address (rs" << static_cast<int>(rs1) << "): 0x" << std::hex << registers[rs1].data
					  << std::endl;
		...
		case Tag::SH: {
			if (addr + 1 >= MEMORY_SIZE_BYTES) {
				std::cerr << "ERROR: Store halfword instruction out of bounds at address 0x" << std::hex << addr
						  << std::endl;
				std::exit(1);
			}
			memory[addr] = static_cast<uint8_t>(value);
		...
		case Tag::SW: {
			if (addr + 3 >= MEMORY_SIZE_BYTES) {
				std::cerr << "ERROR: Store word instruction out of bounds at address 0x" << std::hex << addr
						  << std::endl;
				std::exit(1);
			}
			memory[addr] = static_cast<uint8_t>(value);
