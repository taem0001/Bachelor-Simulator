...
namespace Test::Unit {
    ...
    static bool s_test(uint32_t a_data, Simulator::Tag a_tag, uint32_t b_data, Simulator::Tag b_tag,
                       uint32_t expected_memory, uint8_t imm11_5, uint32_t expected_result) {
        int32_t imm = ((imm11_5 & 0x7F) << 5) | 0b00101;
        if (imm & 0x800) {
            imm |= ~0xFFF;
        }
        ...
        case Simulator::Tag::SW:
            width = 4;
            break;
        }

        int addr = static_cast<int64_t>(a_data) + imm;
        if (addr < 0 || addr + width > MEMORY_SIZE_BYTES) {
            return false;
        }

        Simulator::CPU cpu;
        cpu.set_register(5, a_data, a_tag);
        ...
        return static_cast<uint32_t>(addr) == expected_memory && result == expected_result;
    }

    bool STester::i8_test() { return s_test(-8, Simulator::Tag::SB, 20, Simulator::Tag::SB, 29, 0b0000001, 20); }
    ...
    bool STester::ui16_test() { return s_test(42, Simulator::Tag::UH, 1000, Simulator::Tag::UH, 79, 0b0000001, 1000); }
    bool STester::ui32_test() {
        return s_test(123, Simulator::Tag::UW, 10000, Simulator::Tag::UW, 64, 0b1111110, 10000);
    }
} // namespace Test::Unit